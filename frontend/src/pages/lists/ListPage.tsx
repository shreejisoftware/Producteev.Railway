...
import type { Task, TaskStatus, TaskPriority, List, User, Tag } from '../../types';
...
const STATUS_CONFIG: Record<TaskStatus, {
  ...
  PENDING: { label: 'Pending', color: 'text-[#243B53]', dot: 'bg-[#f59e0b]', bg: 'bg-[#f59e0b]', ring: 'ring-[#d97706]', glow: 'shadow-[#f59e0b]/20' },
  IN_PROGRESS: { label: 'In Progress', color: 'text-white', dot: 'bg-[#cb1d63]', bg: 'bg-[#cb1d63]', ring: 'ring-[#a8174f]', glow: 'shadow-[#cb1d63]/30' },
  IN_REVIEW: { label: 'In Review', color: 'text-white', dot: 'bg-[#f97316]', bg: 'bg-[#f97316]', ring: 'ring-[#ea580c]', glow: 'shadow-[#f97316]/30' },
  ACCEPTED: { label: 'Accepted', color: 'text-white', dot: 'bg-[#ef4444]', bg: 'bg-[#ef4444]', ring: 'ring-[#dc2626]', glow: 'shadow-[#ef4444]/30' },
  COMPLETED: { label: 'Completed', color: 'text-white', dot: 'bg-[#000000]', bg: 'bg-[#000000]', ring: 'ring-[#1E293B]', glow: 'shadow-black/20' },
  REJECTED: { label: 'Rejected', color: 'text-white', dot: 'bg-[#8b5cf6]', bg: 'bg-[#8b5cf6]', ring: 'ring-[#7c3aed]', glow: 'shadow-[#8b5cf6]/30' },
  CLOSED: { label: 'Closed', color: 'text-white', dot: 'bg-[#10b981]', bg: 'bg-[#10b981]', ring: 'ring-[#059669]', glow: 'shadow-[#10b981]/30' },
};

function FlagIcon({ color }: { color: string }) {
  return (
    <svg width="14" height="14" viewBox="0 0 20 20" fill={color} className="shrink-0">
...
const PRIORITY_CONFIG: Record<TaskPriority, { label: string; icon: React.ReactNode; color: string; bg: string }> = {
  URGENT: {
    label: 'Urgent',
    icon: <FlagIcon color="#ef4444" />,
    color: 'text-red-500', bg: 'bg-red-50 dark:bg-red-950/40',
  },
  HIGH: {
    label: 'High',
    icon: <FlagIcon color="#f97316" />,
    color: 'text-orange-500', bg: 'bg-orange-50 dark:bg-orange-950/40',
  },
  MEDIUM: {
    label: 'Medium',
    icon: <FlagIcon color="#eab308" />,
    color: 'text-yellow-500', bg: 'bg-yellow-50 dark:bg-yellow-950/40',
  },
  LOW: {
    label: 'Low',
    icon: <FlagIcon color="#6b7280" />,
    color: 'text-slate-400', bg: 'bg-slate-50 dark:bg-slate-800/40',
  },
};

const STATUS_ORDER: TaskStatus[] = ['OPEN', 'IN_PROGRESS', 'IN_REVIEW', 'COMPLETED', 'PENDING', 'ACCEPTED', 'REJECTED', 'CLOSED'];

// Colored circle config matching TaskDetailPage status icons
const STATUS_CIRCLE: Record<TaskStatus, { color: string; type: 'dashed' | 'ring' | 'check' }> = {
  OPEN: { color: '#94a3b8', type: 'dashed' },
  PENDING: { color: '#f59e0b', type: 'ring' },
  IN_PROGRESS: { color: '#cb1d63', type: 'ring' },
  IN_REVIEW: { color: '#f97316', type: 'ring' },
  ACCEPTED: { color: '#ef4444', type: 'ring' },
  COMPLETED: { color: '#000000', type: 'ring' },
  REJECTED: { color: '#8b5cf6', type: 'ring' },
  CLOSED: { color: '#10b981', type: 'check' },
};

function StatusDot({ status, className = 'w-4 h-4' }: { status: TaskStatus; className?: string }) {
  const c = STATUS_CIRCLE[status];
  const ref = useRef<HTMLDivElement>(null);
  const dotRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (ref.current) {
      if (c.type === 'dashed' || c.type === 'ring') {
        ref.current.style.borderColor = c.color;
      } else if (c.type === 'check') {
        ref.current.style.backgroundColor = c.color;
      }
    }
    if (dotRef.current) {
      dotRef.current.style.backgroundColor = c.color;
    }
  }, [c]);

  if (c.type === 'dashed') {
    return <div ref={ref} className={`${className} rounded-full border-2 border-dashed shrink-0`} />;
  }
  if (c.type === 'check') {
    return (
      <div ref={ref} className={`${className} rounded-full flex items-center justify-center shrink-0`}>
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="4"><path d="M20 6L9 17l-5-5" /></svg>
      </div>
    );
  }
  return (
    <div ref={ref} className={`${className} rounded-full border-2 flex items-center justify-center shrink-0`}>
      <div ref={dotRef} className="w-1.5 h-1.5 rounded-full" />
    </div>
  );
}
...
// See: components/common/DatePickerPopup.tsx
...
function TagPill({ color, name }: { color: string; name: string }) {
  ...
  return (
    <span ref={ref} className="shrink-0 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-semibold leading-none border" title={name}>
      <span ref={dotRef} className="w-1.5 h-1.5 rounded-full shrink-0" />
      {name}
    </span>
  );
}
...
export function ListPage() {
  ...
  const [confirmListDeleteTaskId, setConfirmListDeleteTaskId] = useState<string | null>(null);
  const [isListDeleteBusy, setIsListDeleteBusy] = useState(false);
  ...
  const [availableLists, setAvailableLists] = useState<{ spaceId: string; spaceName: string; lists: { id: string; name: string; folderName?: string }[] }[]>([]);
  ...
  // Date picker per task row
  const [datePickerTaskId, setDatePickerTaskId] = useState<string | null>(null);
  const datePickerRef = useRef<HTMLDivElement>(null);

  const { canCreateTask, canDeleteTask, canUpdateTaskDetails, canAssignTask, canUpdateTaskStatus, canUpdateTaskPriority, isReadOnly, isAdmin } = useOrgRole();
  ...
  const confirmListDeleteTask = async () => {
    if (!confirmListDeleteTaskId) return;
    setIsListDeleteBusy(true);
    try {
      await handleDeleteSingleTask(confirmListDeleteTaskId);
      setConfirmListDeleteTaskId(null);
    } finally {
      setIsListDeleteBusy(false);
    }
  };
  ...
  const handleToggleTag = async (taskId: string, tagId: string) => {
    ...
    try {
      const res = await api.patch<{ success: boolean; data: Task }>(`/tasks/${taskId}`, { tagIds: newTagIds });
      setTasks(prev => prev.map(t => t.id === taskId ? { ...t, tags: res.data.data.tags } : t));
    } catch {
      toast.error('Failed to update tags');
    }
  };

  const openMoveDialog = async (taskId: string) => {
    setMoveTaskId(taskId);
    setMoveTargetListId(null);
    setMoveSearch('');
    try {
      const res = await api.get<{ success: boolean; data: any[] }>(`/spaces/my?orgId=${currentOrg?.id}`);
      const spaces = res.data.data || [];
      const grouped = spaces.map((space: any) => {
        const lists: { id: string; name: string; folderName?: string }[] = [];
        (space.lists || []).forEach((l: any) => {
          if (l.id !== id) lists.push({ id: l.id, name: l.name });
        });
        (space.folders || []).forEach((folder: any) => {
          (folder.lists || []).forEach((l: any) => {
            if (l.id !== id) lists.push({ id: l.id, name: l.name, folderName: folder.name });
          });
        });
        return { spaceId: space.id, spaceName: space.name, lists };
      }).filter((s: any) => s.lists.length > 0);
      setAvailableLists(grouped);
    } catch {
  ...
  const handleMoveTask = async () => {
    if (!moveTaskId || !moveTargetListId) return;
    setMoveLoading(true);
    try {
      await api.patch(`/tasks/${moveTaskId}`, { listId: moveTargetListId });
  ...
  const pendingListDeleteTitle = confirmListDeleteTaskId
    ? tasks.find(t => t.id === confirmListDeleteTaskId)?.title
    : undefined;
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-[#0F172A] overflow-hidden">
      ...
      <div className="shrink-0 border-b border-gray-100 dark:border-gray-800/80">
        ...
        <div className="px-6 pt-5 pb-0">
          ...
          <div className="relative flex items-center">
            <div className="flex-1 overflow-x-auto scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-700 [-ms-overflow-style:none] [scrollbar-width:thin]">
              <div className="flex items-center gap-0.5 whitespace-nowrap pr-28">
                {([
                  ...
                  { id: 'calendar', label: 'Calendar', icon: <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><rect x="3" y="4" width="18" height="18" rx="2" /><path d="M16 2v4M8 2v4M3 10h18" /></svg> },
                  { id: 'gantt', label: 'Gantt', icon: <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path d="M4 6h10M4 12h16M4 18h6" /></svg> },
                ] as const).map(tab => (
                  <button
                    key={tab.id}
            ...
            <div className="absolute right-0 top-1/2 -translate-y-1/2 pl-3 bg-white dark:bg-[#0F172A]">
              <GlobalTimer />
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        {viewMode === 'board' && (
          <BoardView tasks={filteredTasks} onTaskStatusChange={handleTaskStatusChange} />
        )}
        ...
        {viewMode === 'gantt' && (
          <GanttView tasks={filteredTasks} onTasksChange={loadData} />
        )}
          ...
          <>
            ...
            <div className="px-0 py-0 space-y-6 min-w-[1200px]">
              {STATUS_ORDER.map(status => {
                ...
                if (stTasks.length === 0 && addingInStatus !== status) return null;
                      ...
                      {!isCollapsed && (
                        <div className="">
                          <div>
                            ...
                            {stTasks.length > 0 && (
                              <div className="relative flex items-center pl-20 pr-[250px] py-1.5 text-sm font-medium text-gray-400 dark:text-gray-500">
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="w-[52%] flex items-center gap-2 min-w-0 pr-2">
                                    <div className="relative" ref={statusDropdownTaskId === task.id ? statusDropdownRef : undefined}>
                                      <button
                                        onClick={e => { e.stopPropagation(); if (canUpdateTaskStatus) setStatusDropdownTaskId(statusDropdownTaskId === task.id ? null : task.id); }}
                                        className={`flex items-center ${canUpdateTaskStatus ? 'cursor-pointer hover:scale-125' : 'cursor-default'} transition-transform`}
                                        title={canUpdateTaskStatus ? 'Change status' : task.status}
                                      >
                                        <TaskStatusPie status={task.status} className="w-[14px] h-[14px]" />
                                      </button>
                                      {statusDropdownTaskId === task.id && (
                                        <div className="absolute left-0 top-full mt-1 w-[220px] bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-2xl z-[200] overflow-hidden p-1.5" onClick={e => e.stopPropagation()}>
                                          {/* Search */}
                                          ...
                                          <div className="max-h-[300px] overflow-y-auto">
                                            {/* Statuses section */}
                                            <div className="mb-1">
                                              <div className="px-3 py-1.5 text-[11px] font-semibold text-gray-400 dark:text-gray-500">Statuses</div>
                                              {STATUS_ORDER.filter(s => s !== 'CLOSED').filter(s => STATUS_CONFIG[s].label.toLowerCase().includes(statusSearchQuery.toLowerCase())).map(s => {
                                                const sCfg = STATUS_CONFIG[s];
                                                return (
                                                  <button
                                                    key={s}
                                                    onClick={e => { e.stopPropagation(); handleTaskStatusChange(task.id, s); setStatusDropdownTaskId(null); setStatusSearchQuery(''); }}
                                                    className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all mb-0.5 ${task.status === s
                                                        ? 'bg-gray-100/80 dark:bg-gray-700/50'
                                                        : 'hover:bg-gray-50 dark:hover:bg-gray-700/30'
                                                      }`}
                                                  >
                                                    <div className="flex items-center gap-3">
                                                      <StatusDot status={s} className="w-4 h-4" />
                                                      <span className={`text-[13px] uppercase tracking-tight text-gray-700 dark:text-gray-200 ${task.status === s ? 'font-black' : 'font-medium'}`}>
                                                        {sCfg.label}
                                                      </span>
                                                    </div>
                                                    {task.status === s && (
                                                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" className="text-gray-800 dark:text-gray-200"><path d="M20 6L9 17l-5-5" /></svg>
                                                    )}
                                                  </button>
                                                );
                                              })}
                                              ...
                                              {STATUS_ORDER.filter(s => s === 'CLOSED').filter(s => STATUS_CONFIG[s].label.toLowerCase().includes(statusSearchQuery.toLowerCase())).map(s => {
                                                ...
                                                return (
                                                  <button
                                                    key={s}
                                                    onClick={e => { e.stopPropagation(); handleTaskStatusChange(task.id, s); setStatusDropdownTaskId(null); setStatusSearchQuery(''); }}
                                                    className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all ${task.status === s
                                                        ? 'bg-gray-100/80 dark:bg-gray-700/50'
                                                        : 'hover:bg-gray-50 dark:hover:bg-gray-700/30'
                                                      }`}
                                                  >
                                    ...
                                    {/* Add/Edit tag button */}
                                    <div className="relative" ref={tagPickerTaskId === task.id ? tagPickerRef : undefined}>
                                      <button
                                        onClick={e => { e.stopPropagation(); setTagPickerTaskId(tagPickerTaskId === task.id ? null : task.id); }}
                                        className="opacity-0 group-hover/row:opacity-100 w-5 h-5 flex items-center justify-center rounded-md text-gray-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all"
                                        title="Add tag"
                                      >
                                      ...
                                      {tagPickerTaskId === task.id && (
                                        <div className="absolute left-0 top-full mt-1 z-[200]" onClick={e => e.stopPropagation()}>
                                          <TagPicker
                                            organizationId={currentOrg?.id || task.list?.space?.organizationId || task.project?.organizationId || ''}
                                  ...
                                  <div className="w-[140px] flex items-center justify-center shrink-0">
                                    <div onClick={e => e.stopPropagation()} ref={datePickerTaskId === task.id ? datePickerRef : undefined} className="relative group/date rounded-md border border-transparent hover:border-gray-200 dark:hover:border-gray-600 px-1 py-0.5 transition-all cursor-pointer flex items-center gap-0.5">
                                      <button
                                        onClick={e => {
                                          e.stopPropagation();
                                          if (canUpdateTaskDetails) setDatePickerTaskId(datePickerTaskId === task.id ? null : task.id);
                                        }}
                                      ...
                                      {dueParsed && canUpdateTaskDetails && (
                                        <button
                                          onClick={(e) => { e.stopPropagation(); handleUpdateDueDate(task.id, ''); setDatePickerTaskId(null); }}
                                      ...
                                      {datePickerTaskId === task.id && (
                                        <DatePickerPopup
                                          ...
                                          onChange={(iso) => handleUpdateDueDate(task.id, iso)}
                                          onClear={() => handleUpdateDueDate(task.id, '')}
                                  ...
                                  <div className="w-[120px] flex justify-center shrink-0">
                                    <div onClick={e => e.stopPropagation()} className="rounded-md border border-transparent hover:border-gray-200 dark:hover:border-gray-600 px-1 py-0.5 transition-all cursor-pointer">
                                      <Dropdown
                                        trigger={
                                        <button 
                                          className={`transition-transform ${canUpdateTaskPriority ? 'hover:scale-105 active:scale-95 cursor-pointer' : 'cursor-default'}`} 
                                          title={canUpdateTaskPriority ? 'Change priority' : task.priority}
                                          onClick={e => !canUpdateTaskPriority && e.stopPropagation()}
                                        >
                                      ...
                                      >
                                        {(['URGENT', 'HIGH', 'MEDIUM', 'LOW'] as TaskPriority[]).map(pr => (
                                          <DropdownItem
                                            ...
                                            icon={<span className={PRIORITY_CONFIG[pr].color}>{PRIORITY_CONFIG[pr].icon}</span>}
                                          >
                                  ...
                                  <div className="w-[32px] flex items-center justify-center shrink-0" onClick={e => e.stopPropagation()}>
                                    ...
                                    >
                                      ...
                                      <DropdownItem
                                        icon={<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>}
                                        onClick={() => openMoveDialog(task.id)}
                                      >
                                        Move
                                      </DropdownItem>
                                      {canDeleteTask && (
                                        <>
                                          ...
                                          <DropdownItem
                                            ...
                                            onClick={() => setConfirmListDeleteTaskId(task.id)}
                                          >
                                            Delete
                                          </DropdownItem>
                                        </>
                            ...
                            {canCreateTask && (
                              <div className="px-5 py-1.5">
                                {addingInStatus === status ? (
                                  <div className="flex items-center gap-2.5 bg-violet-50 dark:bg-violet-900/15 border border-violet-200 dark:border-violet-800/50 rounded-lg px-3 py-2">
                                    <input
                                      type="text"
                                      value={newTaskTitle}
        ...
        {moveTaskId && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-[420px] max-h-[500px] flex flex-col"
              onClick={e => e.stopPropagation()}
            >
              <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200">Move Task to Another List</h3>
                <p className="text-[11px] text-gray-400 mt-0.5">Select the list you want to move this task to.</p>
              </div>
              ...
              <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-800">
                <input
                  ...
                  className="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-300 outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400"
                  autoFocus
                />
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-3 py-2 min-h-0">
                {availableLists.length === 0 ? (
                  <div className="text-center py-8 text-sm text-gray-400">Loading lists...</div>
                ) : (
                  availableLists.map(space => {
                    const filtered = space.lists.filter(l =>
                      l.name.toLowerCase().includes(moveSearch.toLowerCase()) ||
                      (l.folderName && l.folderName.toLowerCase().includes(moveSearch.toLowerCase()))
                    );
                    if (filtered.length === 0) return null;
                    return (
                      <div key={space.spaceId} className="mb-3">
                        <div className="px-2 py-1.5 text-[10px] font-black text-gray-400 uppercase tracking-wider">{space.spaceName}</div>
                        {filtered.map(l => (
                          <button
                            key={l.id}
                            onClick={() => setMoveTargetListId(l.id)}
                            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-all ${moveTargetListId === l.id ? 'bg-indigo-50 dark:bg-indigo-900/30 ring-2 ring-indigo-500/40' : 'hover:bg-gray-50 dark:hover:bg-gray-800'}`}
                          >
                            <svg className="w-4 h-4 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                            <div className="min-w-0 flex-1">
                              <div className="text-[13px] font-semibold text-gray-700 dark:text-gray-200 truncate">{l.name}</div>
                              {l.folderName && <div className="text-[10px] text-gray-400 truncate">{l.folderName}</div>}
                            </div>
                            {moveTargetListId === l.id && (
                              <svg className="w-4 h-4 text-indigo-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}><polyline points="20 6 9 17 4 12" /></svg>
                            )}
                          </button>
                        ))}
                      </div>
              ...
              <div className="px-5 py-3 border-t border-gray-100 dark:border-gray-800 flex items-center justify-end gap-2">
                <button
                  onClick={() => setMoveTaskId(null)}
                  className="px-4 py-2 text-xs font-bold text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={handleMoveTask}
                  disabled={!moveTargetListId || moveLoading}
                  className={`px-5 py-2 text-xs font-bold rounded-lg transition-all ${moveTargetListId && !moveLoading ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed dark:bg-gray-800'}`}
                >
                  {moveLoading ? 'Moving...' : 'Move Task'}
                </button>
              </div>
            </motion.div>
          </motion.div>
      ...
      <ConfirmDialog
        open={!!confirmListDeleteTaskId}
        title="Are you sure?"
        description={pendingListDeleteTitle ? `Do you want to delete "${pendingListDeleteTitle}"?` : 'Do you want to delete this task?'}
        confirmText="Delete"
        cancelText="Cancel"
        tone="danger"
        isBusy={isListDeleteBusy}
        onClose={() => { if (!isListDeleteBusy) setConfirmListDeleteTaskId(null); }}
        onConfirm={confirmListDeleteTask}
      />
    </div>
  );
}
